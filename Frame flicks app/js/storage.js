...
import { DEFAULT_CONTENT_TYPES } from './format.js';
...
const KEYS = {
  ...
  clients: 've_ct_clients',
  contentTypes: 've_ct_content_types',
  meetings: 've_ct_meetings',
};
...
/* ---------------- Clients (shared by Outreach "Closed" prospects and
   manually-added Dashboard clients — every project links to one of these) */
export function getClients() {
  return fbLoad(KEYS.clients, []);
}
export function saveClients(list) {
  return fbSave(KEYS.clients, list);
}

/* ---------------- Content Types (Client mode only — managed on their own
   page, used as the dropdown when adding/editing a project) ---------------- */
export function getContentTypes() {
  return fbLoad(KEYS.contentTypes, DEFAULT_CONTENT_TYPES);
}
export function saveContentTypes(list) {
  return fbSave(KEYS.contentTypes, list);
}

/* ---------------- Meetings (Calendar — the one thing you add manually) --- */
export function getMeetings() {
  return fbLoad(KEYS.meetings, []);
}
export function saveMeetings(list) {
  return fbSave(KEYS.meetings, list);
}
