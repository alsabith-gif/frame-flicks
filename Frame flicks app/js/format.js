...
export const OPTIONAL_STAGE_GROUPS = ['Color & Sound', 'Motion & VFX', 'AI Elements'];
...
// The step circles on the client's progress page (CPPP) should follow the
// exact same "only show what's actually relevant" rule as the checklist:
// if only one service in a group is ticked, the step's label is that
// service's own name, not the combined group name. The stage's *identity*
// (used for matching current-stage / stage_history keys) stays the fixed
// group name — only what's displayed to the client changes.
export function getStageLabel(services, stageKey) {
  if (!OPTIONAL_STAGE_GROUPS.includes(stageKey)) return stageKey;
  const members = SERVICE_OPTIONS.filter((s) => s.group === stageKey && isServiceTicked(services, s.key));
