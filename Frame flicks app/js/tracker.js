...
// mailto: links silently do nothing on phones with no default mail app set,
// or when the link was opened inside another app's built-in browser (e.g.
// WhatsApp/Instagram), which often blocks them outright. As a safety net,
// tapping Email also copies the address so the client always gets it even
// if their mail app never opens.
function wireEmailFallback() {
  ...
  emailBtn.addEventListener('click', async () => {
    try {
      ...
      // clipboard API unavailable — mailto: is still attempted normally.
    }
  });
}
