...
import { getServiceChecklist, getStageLabel } from './format.js';
...
function renderSteps(currentStage, stageHistory, stages, services) {
  ...
  stepsEl.innerHTML = projectStages.map((stage, i) => {
    ...
    return `
      <div class="track-step" data-i="${i}">
        ...
        <div class="track-step-label">${escHtml(getStageLabel(services, stage))}</div>
  ...
  try {
    ...
    if (!isRefresh || stageChanged || data.note !== lastNote) {
      ...
      if (stage === 'Delivered') {
        ...
        renderSteps(stage, data.stage_history, data.stages, data.services);
        if (data.due_date) {
      ...
      const servicesBlock = document.getElementById('trackServicesBlock');
      const checklist = getServiceChecklist(data.services);
      if (checklist.length) {
        document.getElementById('trackServicesList').innerHTML = checklist.map((item) => `
          <div class="track-service-row ${item.complete ? 'complete' : ''}">
            <span class="track-service-check">${item.complete ? '<svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>' : ''}</span>
            <span class="track-service-label">${escHtml(item.label)}</span>
            <span class="track-service-status">${item.complete ? 'Complete' : 'In progress'}</span>
          </div>`).join('');
        servicesBlock.style.display = '';
      } else {
