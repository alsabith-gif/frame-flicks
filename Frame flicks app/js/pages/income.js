...
function revisionsBadgeHtml(e) {
  ...
  return `<button class="btn btn-icon${over ? ' rev-over' : ''}" data-action="rev-plus" data-id="${e.id}" title="${over ? 'Over the free-revision budget — tap to log another' : 'Tap to log a revision round'}">
    <span class="rev-badge ${over ? 'badge-red' : 'badge-muted'}">${used}/${limit}${over ? ' ⚠️' : ''}</span>
  </button>`;
}

/* ---------------- Priority badge/select ---------------- */
function prioritySelectHtml(e) {
  const p = e.priority || 'Normal';
  return `<select class="inline-select priority-select badge ${PROJECT_PRIORITY_CLASS[p] || 'badge-muted'}" data-id="${e.id}">
    ${PROJECT_PRIORITIES.map((opt) => `<option value="${opt}" ${p === opt ? 'selected' : ''}>${opt}</option>`).join('')}
  </select>`;
}
...
function renderTable() {
  ...
  if (!rows.length) {
    ...
    mobileWrap.innerHTML = rows.map((e) => {
      ...
      <div class="mcard">
        <div class="mcard-top">
          <div class="mcard-name">${escHtml(e.client)} — ${escHtml(e.project)}${e.contentType ? `<div class="cell-sub">${escHtml(e.contentType)}</div>` : ''}</div>
          <select class="inline-select proj-status-select badge ${statusCls[e.status] || 'badge-muted'}" data-id="${e.id}">
            ${statusOptions.map((s) => `<option value="${s}" ${e.status === s ? 'selected' : ''}>${s}</option>`).join('')}
          </select>
        </div>
        ...
        <div class="mcard-row"><span>Priority</span>${prioritySelectHtml(e)}</div>
  ...
  function wireActions(root) {
    ...
    root.querySelectorAll('.priority-select').forEach((sel) => {
      sel.addEventListener('change', () => {
        const list = getIncome();
        const entry = list.find((x) => x.id === sel.dataset.id);
        if (!entry) return;
        entry.priority = sel.value;
        saveIncome(list);
