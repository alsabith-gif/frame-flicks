...
function wireRefresh() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      const { pullAllFromCloud } = await import('./cloud.js');
      const { showToast } = await import('./toast.js');
      await pullAllFromCloud();
      showPage(currentModeDefaultOrActivePage());
      updatePipelineMini();
      showToast('Synced');
    } catch (err) {
      console.error(err);
...
// Re-shows whatever page is currently on screen (by reading the active nav
// item), so refresh re-renders the same place the user is on.
function currentModeDefaultOrActivePage() {
  const activeBtn = document.querySelector('.nav-item.active');
  return activeBtn ? activeBtn.dataset.page : DEFAULT_PAGE[currentMode];
}
